    ...
    def return_matches(self, hashes):
        ...
        with self.cursor() as cur:
            for split_values in grouper(values, 1000):
                ...
                list_split_values = list(split_values)
                query = self.SELECT_MULTIPLE
                query = query % ', '.join(['UNHEX(%s)'] * len(list_split_values))
                cur.execute(query, list_split_values)
                #print("10 values")
                #print(list_split_values[0:10])
                #print("len")
                print(len(list_split_values))
                for hash, sid, offset in cur:
                    # (sid, db_offset - song_sampled_offset)
                    print("hash = " + str(hash) + "sid = " + str(sid))
